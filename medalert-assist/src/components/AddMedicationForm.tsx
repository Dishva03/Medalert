...
import { ChevronRight, ChevronLeft, Plus, AlertCircle, Loader2 } from "lucide-react";
import { format } from "date-fns";
import { Medication as MedicationData } from "@/services/medication.service";
...
const AddMedicationForm = ({ onAddMedication, onCancel }: AddMedicationFormProps) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Pass the medication to parent component for handling
      ...
      addNotification(
        "Medication Added",
        `${formData.name} has been added to your schedule for ${formData.time}`,
        "medicine-added"
      );
      
      // Speak confirmation
      speak(`${formData.name} has been added to your schedule for ${formData.time}`);
    } catch (err) {
    ...
    <div className="w-full max-w-3xl mx-auto bg-card rounded-xl shadow-lg border border-border p-6">
      ...
      {step === 2 && (
        <div className="space-y-6">
          ...
          <div>
            <Label htmlFor="notes" className="text-lg font-medium">Special Instructions (Optional)</Label>
            <Input
              id="notes"
              name="notes"
